...
module.exports={
  ...
  async userStats(req,res,next){
    ...
    const user = await User.findById(req.params.id).populate('locationsVisited').lean()
    ...
    locationsVisited.map((location) => {
      location.visitedBy.map((visit) => { 
        if(visit.userId.toString()===req.params.id){
           userVisit.push({...visit,title:location.title,locationId:location._id})
        }
      })
    })
    //sort array by date
